...
const void* mymemchr(const void* s, int c, size_t n) {
  const uint8_t* ss = reinterpret_cast<const uint8_t*>(s);
  uint8_t cc = c;
...
#ifdef TEST_MAIN_BASE32
#include <stdio.h>
int main(int argc, char* argv[]) {
  char buf[80];
  char buf2[80];
  size_t olen = 80;
  base32_encode(argv[1], strlen(argv[1]), buf, 80);
  base32_decode(buf, strlen(buf), buf2, &olen);
  buf2[olen] = 0;
  printf("%d %s\n%d %s\n", (int)strlen(buf), buf, (int)olen, buf2);
}
#endif
